...
def train(
    ...
    with mlflow.start_run():
        ...
        prec = precision_score(y_eval, preds, average="weighted", zero_division=0)
        rec = recall_score(y_eval, preds, average="weighted", zero_division=0)
        ...
        # Note: mlflow.sklearn.log_model requires HTTP tracking server (not sqlite)
        # Model is saved separately with joblib below
        ...
        with open("outputs/metrics.json", "w") as f:
            json.dump({"accuracy": acc, "f1_score": f1, "precision": prec, "recall": rec}, f)
